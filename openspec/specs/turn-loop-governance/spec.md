...
## Purpose
Bounds and guides the per-turn agent loop. This capability owns the limits and
guards that stop a turn from looping, stalling, or burning context: iteration
caps, tool-error follow-up nudges, the per-response thinking cap, and the
plan-without-action re-prompt.
## Requirements
