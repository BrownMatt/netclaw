...
The GUI SHALL list all sessions from the daemon session catalog with title,
channel, and activity ordering. A click SHALL attach the selected session
and render its replayed recent history. `session_title` events SHALL update
the list live. The list SHALL offer no rename, pin, archive, or delete
action in this capability.
